...
PRIORITY_IMAGES_PER_NON_PRIORITY_IMAGE = 3
...
def next_downlink_image(
    priority_queue_dir,
    non_priority_queue_dir,
    no_ship_detected_dir,
    consecutive_priority_images,
):
    priority_image = next_image(priority_queue_dir)
    non_priority_image = next_image(non_priority_queue_dir)

    if priority_image and (
        not non_priority_image
        or consecutive_priority_images < PRIORITY_IMAGES_PER_NON_PRIORITY_IMAGE
    ):
        return priority_image, consecutive_priority_images + 1
    if non_priority_image:
        return non_priority_image, 0
